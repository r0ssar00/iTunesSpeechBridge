...
namespace RCF {
    ...
    public:
        ...
        { 
            if (in.getRemainingArchiveLength() != 0)
            {
                ...
                RCF_VERIFY(
                    pt,
                    RCF::Exception(RcfError_DeserializationNullPointer))
                    (typeid(T));
    ...
    RCF_EXPORT Candidates & gCurrentCandidates();
        ...
        bool enrolFutures(RCF::ClientStub *pClientStub)
        {
            ...
            for (std::size_t i=0; i<sizeof(pva)/sizeof(pva[0]); ++i)
            {
                ...
                std::map<const void *,I_Future *>::iterator iter = 
                    gCurrentCandidates().find(pv);

                if ( iter != gCurrentCandidates().end() ) 
                {
                    pClientStub->enrol( (*iter).second );
                    gCurrentCandidates().erase(iter);
                    enrolled = true;
                }
            }
    ...
    public:
        ...
        {
            mStatePtr->mpt = rhs.mStatePtr->mpt;
            mStatePtr->mpClientStub = rhs.mStatePtr->mpClientStub;
            return *this;
        }
        ...
        public:
            ...
            operator T&()
            {
                ...
                if (mpClientStub)
                {
                    ...
                    {
                        RCF_THROW(Exception(RcfError_FutureDereference));
                    }

                    if (mpClientStub->hasAsyncException())
                    {
                        RCF_THROW(Exception(RcfError_FutureDereference));
                    }
                }
    ...
    public:
        ...
        void assignTo(Future<T> &future) const
        {
            ...
            future.mStatePtr->mpt = mpT;
            future.mStatePtr->mtPtr.reset();
            future.mStatePtr->mpClientStub = mpClientStub;
    ...
    private:
        ...
        void callSync() const
        {
            // For any kind of exception other than a RemoteException, we
            // want the connection reset.
            ConnectionResetGuard connectionResetGuard(*mpClientStub);
            try
            {
