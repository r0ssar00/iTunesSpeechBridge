...
namespace RCF {
    ...
    void AmiThreadPool::cycleIo()
    {
        ...
        while (!mStopFlag)
        {
            try
            {
                ...
                {
                    RCF::Exception e(RcfError_Socket, dwErr);
                    pAmi->onError(e);
                }
            }
            catch(...)
    ...
    void AmiThreadPool::start(std::size_t threadCount)
    {
        ...
        gAmiThreadPool.mStopFlag = false;

        gAmiThreadPool.mIoThreadPtr.reset( new RCF::Thread( boost::bind(
            &AmiThreadPool::cycleIo,
            &gAmiThreadPool)));

        gAmiThreadPool.mTimerThreadPtr.reset( new RCF::Thread( boost::bind(
            &AmiThreadPool::cycleTimer,
            &gAmiThreadPool)));

        gAmiThreadPool.mConnectThreadPtr.reset( new RCF::Thread( boost::bind(
    ...
    {
        gAmiThreadPool.mStopFlag = true;

        gAmiThreadPool.mIoThreadPtr->join();
        gAmiThreadPool.mIoThreadPtr.reset();

        gAmiThreadPool.mTimerThreadPtr->join();
        gAmiThreadPool.mTimerThreadPtr.reset();

        gAmiThreadPool.mConnectThreadPtr->join();
        gAmiThreadPool.mConnectThreadPtr.reset();
    }
    ...
    Exception AmiThreadPool::connect(int fd, sockaddr_in & addr, OverlappedAmiPtr overlappedAmiPtr)
    {
        ...
        if (useConnectEx)
        {
            ...
            if (ret != 0)
            {
                return Exception(RcfError_Socket, err);
            }
    ...
    Exception AmiThreadPool::addConnectFd(int fd, OverlappedAmiPtr overlappedAmiPtr)
    {
        ...
        if (mConnectFds.size() >= FD_SETSIZE)
        {
            ...
            return Exception(RcfError_FdSetSize, msg);
        }
    ...
    void AmiThreadPool::cycleConnect()
    {
        ...
        while (!mStopFlag)
        {
            ...
            for (Events::iterator iter = events.begin(); iter != events.end(); ++iter)
            {
                ...
                {
                    RCF::Exception e(RcfError_Socket, error);
