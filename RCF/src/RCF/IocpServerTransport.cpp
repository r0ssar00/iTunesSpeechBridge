...
namespace RCF {
        ...
        int error)
    {
        ...
        mTop ?
            mSessionState.onFilteredReadWriteCompleted(bytesTransferred, error) :
        ...
        int error)
    {
        ...
        if (mPostState == Reading)
        {
            ...
            if (mTransportFilters.empty())
            {
                onFilteredReadWriteCompleted(bytesTransferred, error);
            }
            else
            {
        ...
        DWORD dwNumBytes)
    {
        ...
        IocpSessionState *pSessionState = this;

        IocpSessionStatePtr sessionStatePtr;
        if (pSessionState->mThisPtr)
        {
            sessionStatePtr = pSessionState->mThisPtr;
        ...
        if (    (   ret == TRUE 
            ...
            ||  (   ret == TRUE 
                    && dwNumBytes == 0 
                    && pSessionState->mPreState == IocpSessionState::Accepting)

            ||  (   ret == TRUE 
                    && dwNumBytes == 0 
                    && pSessionState->mIssueZeroByteRead == true 
                    && pSessionState->implIsConnected())
                    ...
                    && pSessionState->mPreState != IocpSessionState::Accepting))
        {
            ...
            RcfSessionPtr rcfSessionPtr = 
                ...
                pSessionState->mSessionPtr);

            SetCurrentSessionGuard guard(rcfSessionPtr);

            int bytesRead = dwNumBytes;
            pSessionState->onReadWriteCompleted(bytesRead, 0);
        }
        else
        {
