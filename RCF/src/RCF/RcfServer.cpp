...
namespace RCF {
        ...
        if (!mStopCallInProgress)
        {
            ...
            if (!ok)
            {
                ...
                bool okToWrite = false;
                {
                    ...
                    {
                        RCF_ASSERT(mIoState == WritingPingBack);
                        mQueuedSendBuffers = byteBuffers;
                    }
                }
    ...
    void RcfSession::sendSessionResponse()
    {
        ...
        if (mRequest.getOneway())
        {
            mOut.extractByteBuffers();
    ...
    void RcfSession::sendResponseUncaughtException()
    {
        ...
        RCF2_TRACE("")(this);
        sendResponse( RemoteException(RcfError_BadException));
    }

    void serializeRemoteException(
        SerializationProtocolOut & out, 
        const RemoteException & e)
    {
        if (
            out.getSerializationProtocol() == BsBinary 
            || out.getSerializationProtocol() == BsText 
            || out.getSerializationProtocol() == BsXml)
        {
            // Boost serialization is very picky about serializing pointers vs values.
            // Since the client will deserialize an auto_ptr, we are forced to 
            // create an auto_ptr here as well.
            std::auto_ptr<RemoteException> apRe(new RemoteException(e));
            serialize(out, apRe);
        }
        else
        {
