...
namespace RCF {
    ...
    bool SspiServerFilter::doHandshakeSchannel()
    {
        ...
        else if (ib[1].BufferType == SECBUFFER_EXTRA)
        {
            ...
            RCF_THROW(Exception(RcfError_SspiHandshakeExtraData));
        }
    ...
    bool SspiClientFilter::doHandshakeSchannel()
    {
        ...
        {
            RCF_THROW(Exception(RcfError_SspiAuthFail, status));
        }
    }
            ...
            mpCertContext(RCF_DEFAULT_INIT)
    {
        ...
        std::size_t bytesRead = fin.read( &buffer[0], buffer.size() ).gcount();
        fin.close();
