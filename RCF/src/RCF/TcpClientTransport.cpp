...
namespace RCF {
        ...
        unsigned int timeoutMs)
    {
        ...
        if (mRemoteAddr.sin_addr.s_addr == INADDR_NONE)
        {
            ...
            if (hostDesc)
            {
                ...
                RCF_VERIFY(szIp, Exception(RcfError_DnsLookup, mIp));
    ...
    void TcpClientTransport::endDnsLookup()
    {
        if (mRemoteAddr.sin_addr.s_addr == INADDR_NONE)
        {
            mClientStubCallbackPtr.onError(Exception(RcfError_DnsLookup, mIp));
            return;
        }
        ...
        RCF::Exception e = gAmiThreadPool.connect(mFd, mRemoteAddr, overlappedPtr);
        if (e.bad())
        {
        ...
        unsigned int timeoutMs)
    {
        ...
        if (!mRegisteredForAmi)
        {
            ...
            {
                gAmiThreadPool.mIocp.AssociateSocket(mFd, mFd);
            }
    ...
    void TcpClientTransport::implClose()
    {
        if (mFd != -1)
        {
            if (mRegisteredForAmi)
            {
                gAmiThreadPool.cancelConnect(mFd);
            }
