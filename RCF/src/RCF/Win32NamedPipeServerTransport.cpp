...
namespace RCF {
    ...
    void Win32NamedPipeSessionState::accept()
    {
        ...
        if ( !ok && mError == WSA_IO_PENDING )
        {
            mError = 0;
            Lock lock(mTransport.mQueuedAcceptsMutex);
            ++mTransport.mQueuedAccepts;
        }
        else if (!ok && mError == ERROR_PIPE_CONNECTED)
        {
            mError = 0;
            Lock lock(mTransport.mQueuedAcceptsMutex);
            ++mTransport.mQueuedAccepts;
        ...
        std::size_t bufferLen)
    {
        ...
        bool readOk = false;
        {
            ...
            if (!mHasBeenClosed)
            {
                ...
                else if (ok && dwBytesRead == 0 && bufferLen == 0)
                {
                    readOk = true;
                }
                else if (
    ...
    Win32NamedPipeServerTransport::createSessionState()
    {
        ...
        RCF_VERIFY(hPipe != INVALID_HANDLE_VALUE, Exception(RcfError_Pipe, dwErr));
    ...
    void Win32NamedPipeImpersonator::impersonate()
    {
        ...
        DWORD dwErr = GetLastError();
        RCF_VERIFY(ok, RCF::Exception(RcfError_Pipe, dwErr));
    }
