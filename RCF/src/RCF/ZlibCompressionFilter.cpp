...
namespace RCF {
        ...
        RCF_DTOR_BEGIN
            if (mDecompressionStateInited)
            {
                ...
                RCF_VERIFY(
                    mZerr == Z_OK,
                    FilterException(
                        RcfError_Zlib, mZerr, RcfSubsystem_Zlib,
        ...
        std::size_t bytesRequested)
    {
        if (byteBuffer.isEmpty() && bytesRequested == 0)
        {
            ...
            {
                mFilter.getPreFilter().onReadCompleted(byteBuffer, 0);
            }
            else
            {
        ...
        int error)
    {
        ...
        {
            RCF_ASSERT(byteBuffer.isEmpty() && error == 0);
            mFilter.getPreFilter().onReadCompleted(ByteBuffer(), 0);
        }
        else 
        {
            ...
            {
                mOrigBuffer.clear();
                mFilter.getPreFilter().onReadCompleted(mPreBuffer, 0);
            }
            else
            {
                mPreBuffer.clear();
    ...
    bool ZlibCompressionReadFilter::decompress()
    {
        ...
        RCF_VERIFY(
            ...
            FilterException(
                RcfError_ZlibInflate, mZerr, RcfSubsystem_Zlib,
                "inflate() failed"))
        ...
        int error)
    {
        ...
        else
        {
            ...
            mFilter.getPreFilter().onWriteCompleted(mTotalBytesIn, 0);
        }
    }
