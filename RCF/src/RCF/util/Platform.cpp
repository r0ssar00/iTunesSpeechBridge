...
namespace Platform {
    namespace OS {
        ...
        boost::uint32_t getCurrentTimeMs()
        {
            ...
            boost::uint64_t seconds =  
                boost::uint64_t(now.tv_sec) - boost::uint64_t(start.tv_sec);

            boost::uint64_t microseconds = 
                now.tv_sec - start.tv_sec;

            boost::uint64_t timeMs = seconds*1000 + microseconds/1000;
